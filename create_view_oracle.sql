...
select * from view_kontrole;
drop view view_kontrole;


create or replace view view_kontrole_week (id, numer_rejestracyjny, marka, imie_wlasciciela, nazwisko_wlasciciela, przebieg, wynik, lpg, hak, 
    data_kontroli, data_nastepnej_kontroli, cena, stan_silnika, stan_hamulcy, stan_amortyzatorow, stan_zawieszenia, stan_zarowek, stan_lamp) 
	as (SELECT k.k_id, s.s_numer_rejestracyjny, s.s_marka, i.if_imie, i.if_nazwisko, dp.dp_przebieg, k.k_pozytywny, k.k_lpg, k.k_hak, k.k_data_kontroli, 
    k.k_data_nastepnej_kontroli, k.k_cena, sp.sp_silnik, sp.sp_hamulce, sp.sp_amortyzatory, sp.sp_zawieszenie, sp.sp_zarowki, sp.sp_lampy
	FROM kontrole k, samochody s, dane_pojazdow dp, stany_pojazdow sp, wlasciciele w, informacje_osobowe i 
    WHERE k.s_id = s.s_id AND s.dp_id = dp.dp_id AND s.sp_id = sp.sp_id AND s.w_id = w.w_id AND w.if_id = i.if_id AND k.k_data_kontroli > (sysdate - 7));
select * from view_kontrole_week;
...
create or replace view view_utarg (id_placowki, kod_pocztowy, miasto, ulica, nr, data, utarg_dzienny) 
	as (SELECT DISTINCT pl.pl_id, ad.ad_kod_pocztowy, ad.ad_miasto, ad.ad_ulica, ad.ad_nr_domu, k.k_data_kontroli, SUM(k_cena) OVER(PARTITION BY k.k_data_kontroli)
	FROM placowki pl, pracownicy p, kontrole k, adresy ad WHERE pl.ad_id = ad.ad_id AND pl.pl_id = p.pl_id 
	AND p.p_id = k.p_id);
select * from view_utarg;
